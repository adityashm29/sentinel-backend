...
async function searchUserPosts(username) {
    try {
        const responseForID = await fetch(`https://api.x.com/2/users/by/username/${username}?user.fields=description`, { method: 'GET', headers: { Authorization: `Bearer ${BEARER_TOKEN}` } });
        const resp = await responseForID.json();
        console.log(resp);
        const description = resp.data.description;
        const id = resp.data.id;
        const response = await fetch(`https://api.x.com/2/users/${id}/tweets?max_results=10`, { method: 'GET', headers: { Authorization: `Bearer ${BEARER_TOKEN}` } });
        const data = await response.json();
        const previousTweets = data.map((tweet) => tweet.text);
    }
    catch (error) {
