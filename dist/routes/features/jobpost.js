...
async function fetchXUser(username) {
    try {
        ...
        const res = await fetch(`https://api.twitter.com/2/users/by/username/${username}?user.fields=${fields}`, {
            headers: {
...
async function fetchXTweets(userId, max = 20) {
    try {
        const fields = ['created_at', 'public_metrics'].join(',');
        const res = await fetch(`https://api.twitter.com/2/users/${userId}/tweets?max_results=${max}&tweet.fields=${fields}&exclude=retweets,replies`, {
            headers: {
                Authorization: `Bearer ${process.env.X_API_BEARER}`,
            },
        });
...
async function historicalBehaviorAgent(user, tweets) {
    const profile = user
...
async function linguisticPatternAgent(postText) {
...
async function organizationVerificationAgent(postText) {
    ...
    try {
        ...
        const extractResult = await extractModel.generateContent(`Extract only the company/organization name from this job post. Return ONLY the company name, nothing else. If no clear company is mentioned, return "Unknown".\n\nJob Post:\n${postText}`);
...
async function reputationNetworkAgent(user, tweets) {
    if (!user) {
        return {
            score: 30,
...
function computeWeightedScore(agents) {
    ...
    const score = Object.entries(weights).reduce((sum, [key, weight]) => {
        return sum + agents[key].score * weight;
    }, 0);
    return Math.round(score);
}
function getVerdict(score) {
    if (score >= 70)
        return { verdict: 'SAFE', verdictLabel: 'This post appears legitimate' };
    if (score >= 40)
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        const [profileConsistency, historicalBehavior, linguisticPattern, organizationVerification, reputationNetwork] = await Promise.all([
        ...
        try {
            ...
            const summaryResult = await summaryModel.generateContent(`Summarize this job post scam analysis in 2-3 sentences for a job seeker. Trust score: ${trustScore}/100. Verdict: ${verdictLabel}. Key caution flags: ${allCautionFlags.slice(0, 3).join('; ')}. Key positive signals: ${allPositiveSignals.slice(0, 3).join('; ')}. Be direct and helpful.`);
